...
class RegisterAndVerifyViewSet(ViewSet):
    ...
    def handle_otp(self, user_instance):
        objs = OTPRegisterResend.objects.filter(
            otp_user=user_instance, otp_type=1, deleted_at=None
        ).order_by('-created_at')
    ...
    def delete_otps(self, user_instance):
        OTPRegisterResend.objects.filter(
            otp_user=user_instance, otp_type=1, deleted_at=None
        ).update(deleted_at=datetime.now())

    def create_and_send_otp(self, user_instance):
        otp = OTPRegisterResend.objects.create(otp_user=user_instance, otp_type=1)
...
class ResendAndResetViewSet(ViewSet):
    ...
    def set_new_password(self, request, *args, **kwargs):
        ...
        otp_obj = OTPSetPassword.objects.filter(otp_token=token, deleted_at=None).first()
        if not otp_obj:
    ...
    def resend_otp_code(self, request, *args, **kwargs):
        ...
        if otp_status == 'limit_exceeded':
            ...
            OTPRegisterResend.objects.filter(
                otp_user=otp_obj.otp_user, otp_type=1, deleted_at=None
            ).update(deleted_at=datetime.now())
